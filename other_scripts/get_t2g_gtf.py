...
tabnames = tabnames.loc[tabnames.iloc[:,2]=="transcript",:]

gene_transcript = [x[0:2] for x in tabnames.iloc[:,8].str.split("; ")]
# get the gene ID
...
for i in range(0,len(annot_gene)):
    ...
    if "_" in annot_gene[i]:
        annot_gene[i] = annot_gene[i].replace("_", "..")
    if "[" in annot_gene[i]:
        annot_gene[i] = annot_gene[i].split(":")[-1].strip("]")
